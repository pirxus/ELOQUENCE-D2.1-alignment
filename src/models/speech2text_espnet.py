...
class Conv2dSubsampler(torch.nn.Module):
    ...
    def forward(self, x, x_mask):
        ...
        if x_mask is None:
            return x, None
        x_mask = x_mask.unsqueeze(1)[:, :, :-2:2][:, :, :-2:2].squeeze()
        return x, x_mask
...
class Speech2TextEncoder(Speech2TextPreTrainedModel):
    ...
    def __init__(self, config: Speech2TextConfig):
        ...
        self.conv = Conv2dSubsampler(config.input_feat_per_channel, embed_dim)
    ...
    def forward(
        ...
        inputs_embeds, attention_mask = self.conv(input_features, attention_mask)
...
class Speech2TextModel(Speech2TextPreTrainedModel):
    ...
    def forward(
        ...
        if attention_mask is not None:
                    ...
                    #)
            _, encoder_attention_mask = self.encoder.conv(None, attention_mask)
        else:
            encoder_attention_mask = None
...
class Speech2TextForConditionalGeneration2d(Speech2TextPreTrainedModel):
    base_model_prefix = "model"
