"""Main training script for the ECD architecture"""
import sys
...
from utilities.collators import SpeechAlignedCollatorWithPadding, SpeechCollatorWithPadding
...
if __name__ == "__main__":
    ...
    training_eval_dataset = dataset['val']
    ...
    if model_args.from_pretrained:
        ...
        model = SpeechEncoderBridgeTextDecoder(encoder=encoder, decoder=decoder, config=apmo_config)

    logger.info(f"Finished loading model {model}")

    # 4. Update generation config
    gen_config = GenerationConfig(
        bos_token_id=decoder.config.decoder_start_token_id if 't5' in training_args.tokenizer_name else tokenizer_target.bos_token_id,
        pad_token_id=tokenizer_target.pad_token_id,
        decoder_start_token_id=decoder.config.decoder_start_token_id if 't5' in training_args.tokenizer_name else (decoder.config.decoder_start_token_id if 'opus' in model_args.base_decoder_model else tokenizer_target.bos_token_id),
